...
from .loss_func_local import *
from .models_local import StoNet
from .data.loader_local import make_dataloader
from .utils_local import *


def engression(x, y, classification=False,
               num_layer=2, hidden_dim=100, noise_dim=100, out_act=None,
               add_bn=True, resblock=False, loss_phi="power", beta=0.5,
    ...
    Args:
        ...
        loss_phi (str): one of ["power","exp","log1p","x_over_1px","power_norm"].
            - "power": φ(x)=x^alpha on x=||·||^2 (RECOMMENDED).
            - "exp":   φ(x)=1 - exp(-x/2)
            - "log1p": φ(x)=log(1+x)
            - "x_over_1px": φ(x)=x/(1+x)
    ...
    engressor = Engressor(in_dim=x.shape[1], out_dim=y.shape[1], classification=classification, 
                          ...
                          out_act=out_act, resblock=resblock, add_bn=add_bn, loss_phi=loss_phi, beta=beta,
...
class Engressor(object):
    ...
    def __init__(self, 
                 ...
                 out_act=False, resblock=False, add_bn=True, loss_phi="power", beta=0.5,
    ...
    def train(self, x, y, num_epochs=None, batch_size=None, lr=None, print_every_nepoch=100, print_times_per_epoch=1, standardize=None, verbose=True):
        ...
        if batch_size >= x.size(0)//2:
            ...
            for epoch_idx in range(self.num_epochs):
                ...
                loss, loss1, loss2 = energy_loss_two_sample(y, y_sample1, y_sample2, phi=self.loss_phi, beta=self.beta, verbose=True)
                loss.backward()
                self.optimizer.step()
    ...
    def train_one_iter(self, x_batch, y_batch):
        ...
        loss, loss1, loss2 = energy_loss_two_sample(y_batch, y_sample1, y_sample2, phi=self.loss_phi, beta=self.beta, verbose=True)
        loss.backward()
        self.optimizer.step()
    ...
    def eval_loss(self, x, y, loss_type="l2", sample_size=None, beta=1, verbose=False):
        ...
        if loss_type == "l2":
            ...
            loss = energy_loss(y, y_samples, phi=self.loss_phi, beta=beta, verbose=verbose)
        if not verbose:
            return loss.item()
        else:
